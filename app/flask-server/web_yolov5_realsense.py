...
def init_realsense(process_que=None):
    ...
    def get_mid_pos(frame, box, depth_data, randnum, depth_intrin):
        ...
        x_list = np.sort(x_list)[randnum // 2 -
                                 randnum // 4:randnum // 2 + randnum // 4]
        y_list = np.array(y_list)
        y_list = np.sort(y_list)[randnum // 2 -
                                 randnum // 4:randnum // 2 + randnum // 4]
        z_list = np.array(z_list)
        z_list = np.sort(z_list)[randnum // 2 -
                                 randnum // 4:randnum // 2 + randnum // 4]
    ...
    def dectshow(org_img, boxs, depth_data, depth_intrin, process_que):
        ...
        for box in boxs:
            ...
            dist, x, y, z = get_mid_pos(
                org_img, box, depth_data, 24, depth_intrin)
    ...
    try:
        while True:
            ...
            info_q = dectshow(color_image, boxs, depth_image,
                              depth_intrin, process_que)
